...
var _logMessages = function _logMessages(reason /*: ReasonT*/) {
  return {
    start: {
      inProgress: "Adtrace SDK ".concat(_disableReason(reason), " process has already started"),
      done: "Adtrace SDK ".concat(_disableReason(reason), " process is now started")
    },
    finish: {
      inProgress: "Adtrace SDK ".concat(_disableReason(reason), " process has already finished"),
      done: "Adtrace SDK ".concat(_disableReason(reason), " process is now finished")
    }
  };
};
...
function _internalTrackEvent(params /*: EventParamsT*/) {
  if (storage.getType() === STORAGE_TYPES.NO_STORAGE) {
    var reason = 'Adtrace SDK can not track event, no storage available';
    logger.log(reason);
    return main_Promise.reject(reason);
  }
  if (disable_status() !== 'on') {
    var _reason = 'Adtrace SDK is disabled, can not track event';
    logger.log(_reason);
    return main_Promise.reject(_reason);
  }
  if (!_isInitialised()) {
    var _reason2 = 'Adtrace SDK can not track event, sdk instance is not initialized';
...
function _preCheck(description /*: string*/, callback /*: () => mixed*/) /*: mixed*/{
  ...
  if (storage.getType() === STORAGE_TYPES.NO_STORAGE) {
    logger.log("Adtrace SDK can not ".concat(description, ", no storage available"));
    return;
  }
  if (disable_status() !== 'on') {
    logger.log("Adtrace SDK is disabled, can not ".concat(description));
    return;
  }
  if (!(optionalInit || _isInitialised()) && waitForInitFinished) {
    logger.error("Adtrace SDK can not ".concat(description, ", sdk instance is not initialized"));
    return;
  }
  if (typeof callback === 'function') {
    if (schedule && !(_isInstalled && _isStarted) && (optionalInit || _isInitialised())) {
      delay(callback, description);
      logger.log("Running ".concat(description, " is delayed until Adtrace SDK is up"));
    } else {
      return callback();
    }
  }
...
/* harmony default export */ const main = (Adtrace);
})();
